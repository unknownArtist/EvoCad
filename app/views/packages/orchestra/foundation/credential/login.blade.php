        ...
        <?php echo Form::open(array('url' => handles('orchestra::login'), 'action' => 'POST', 'class' => 'form-horizontal')); ?>
            <fieldset>
                ...
                <fieldset>
                    <div>
                    <?php
                        $checkedfb = (Setting::get('provider.facebook',0)?true:false);

                        $checkedgp = (Setting::get('provider.google',0)?true:false);

                        $checkedtw = (Setting::get('provider.twitter',0)?true:false);
                        $checkedtw = (Auth::check()?$checkedtw:0);
					?>
					@if($checkedfb)
                        <a href="/login/Facebook" class="btn btn-success">Facebook Login</a>
                    @endif
					@if($checkedgp)
                        <a href="/login/Google" class="btn btn-success">Google + Login</a>
                    @endif
                    @if($checkedtw)
                        <a href="/login/Twitter" class="btn btn-success">Twitter Login</a>
                    @endif
